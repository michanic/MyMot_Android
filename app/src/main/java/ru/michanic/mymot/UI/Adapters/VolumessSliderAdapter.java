...
public class VolumessSliderAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    Context context;
    List<Volume> volumes;
    ClickListener clickListener;

    public VolumessSliderAdapter(Context context, List<Volume> volumes, ClickListener clickListener) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof CatalogSliderCell) {
            ((CatalogSliderCell) holder).fillWithVolume(volumes.get(position));
